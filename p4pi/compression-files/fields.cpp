...
// CHANGED
// function responsible for resizing a VL field.
// ps: we only used for making fields smaller, so I don't know
// if it would work to make them bigger, but nothing excludes that possibility.
void Field::resize_field(int new_nbytes) {
    assert(VL);
    nbytes = (new_nbytes * 8 + 7) / 8;
    nbits = nbytes * 8;
    
    // Resize the byte container to hold the new size of bytes
    bytes.resize(nbytes);

    // Optionally: Call sync_value to ensure internal consistency
    written_to = true;
    if (arith) sync_value();
