...
// CHANGED
ExternConverter_new_addr ExternConverter_new_addr::singleton;
ExternConverter_p4_logger ExternConverter_p4_logger::singleton;
ExternConverter_deflate_payload ExternConverter_deflate_payload::singleton;
ExternConverter_inflate_payload ExternConverter_inflate_payload::singleton;
...
// CHANGED
Util::IJson* ExternConverter_new_addr::convertExternFunction(
    ...
    auto primitive = mkPrimitive("new_addr");
    auto params = mkParameters(primitive);
    primitive->emplace_non_null("source_info", mc->sourceInfoJsonObj());
    auto dest = ctxt->conv->convert(mc->arguments->at(0)->expression);
    auto anything = ctxt->conv->convert(mc->arguments->at(1)->expression);
    params->append(dest);
...
Util::IJson* ExternConverter_p4_logger::convertExternFunction(
    ...
    auto params = mkParameters(primitive);
    primitive->emplace_non_null("source_info", mc->sourceInfoJsonObj());
    auto variable = ctxt->conv->convert(mc->arguments->at(0)->expression);
    params->append(variable);
    return primitive;
}

Util::IJson* ExternConverter_deflate_payload::convertExternFunction(
    UNUSED ConversionContext* ctxt, UNUSED const P4::ExternFunction* ef,
    UNUSED const IR::MethodCallExpression* mc, UNUSED const IR::StatOrDecl* s,
    UNUSED const bool emitExterns) {
    ...
    auto primitive = mkPrimitive("deflate_payload");
    auto params = mkParameters(primitive);
    primitive->emplace_non_null("source_info", mc->sourceInfoJsonObj());
    auto payload_in = ctxt->conv->convert(mc->arguments->at(0)->expression);
    auto payload_in_len = ctxt->conv->convert(mc->arguments->at(1)->expression);
    auto payload_out = ctxt->conv->convert(mc->arguments->at(2)->expression);
    auto payload_out_len = ctxt->conv->convert(mc->arguments->at(3)->expression);
    params->append(payload_in);
    params->append(payload_in_len);
    params->append(payload_out);
    params->append(payload_out_len);
    return primitive;
}

Util::IJson* ExternConverter_inflate_payload::convertExternFunction(
    UNUSED ConversionContext* ctxt, UNUSED const P4::ExternFunction* ef,
    UNUSED const IR::MethodCallExpression* mc, UNUSED const IR::StatOrDecl* s,
    UNUSED const bool emitExterns) {
    if (mc->arguments->size() != 4) {
    ...
    auto primitive = mkPrimitive("inflate_payload");
    auto params = mkParameters(primitive);
    primitive->emplace_non_null("source_info", mc->sourceInfoJsonObj());
    auto payload_in = ctxt->conv->convert(mc->arguments->at(0)->expression);
    auto payload_in_len = ctxt->conv->convert(mc->arguments->at(1)->expression);
...
void
SimpleSwitchBackend::createRecirculateFieldsList(
    ConversionContext* ctxt, const IR::ToplevelBlock* tlb, cstring scalarName) {
